...
class DataPicker extends Component {
                    ...
                    <div className="show_date_title">
                        ...
                        <div className="show_date_title_world">
                            <div onClick={()=>this.setState({year:(this.state.yearStart !== this.state.year)?(this.state.year - 1):this.state.year},()=>{this.getShowDate();})}>
                                <i className="iconfont iconjiantouarrow3" style={{fontSize:"14px",color:"#7d7d7d"}}></i></div>
                            <div onClick={()=>this.setState({month:(1 !== this.state.month)?(this.state.month - 1):this.state.month},()=>{this.getShowDate();})}>
                                <i className="iconfont iconjiantouarrow2" style={{fontSize:"14px",color:"#7d7d7d"}}></i></div>
                            <select value={this.state.year} defaultValue={this.state.year}
                                className="my_select"
                            ...
                            年
                            <select value={this.state.month} defaultValue={this.state.year}
                                className="my_select"
                            ...
                            <div onClick={()=>this.setState({month:(12 !== this.state.month)?(this.state.month + 1):this.state.month},()=>{this.getShowDate();})}>
                                <i className="iconfont iconjiantouarrow1" style={{fontSize:"14px",color:"#7d7d7d"}}></i>
                            </div>
                            <div onClick={()=>this.setState({year:(this.state.yearEnd !== this.state.year)?(this.state.year + 1):this.state.year},()=>{this.getShowDate();})}>
                                <i className="iconfont iconjiantouarrow4" style={{fontSize:"14px",color:"#7d7d7d"}}></i>
                            </div>
                        </div>
                        {/*按钮*/}
                    ...
                    <div className="show_date_main">
                        {
                            this.state.dayList.map((item,index)=>{
                                return(
                                    <div className={`${(Object.is(item.type,"this"))?"thisMouth":""} ${(Object.is(item.type,"thisDay"))?"thisDay":""}`}
    ...
    render() {
        return (
            <div style={{display:"flex"}} className="date_picker">
                <div className="date_picker_title">{this.props.title}</div>
                <div className="date_picker_main">
                    {/*输入框*/}
                    <input value={this.state.value} readOnly="readonly" onChange={this.onChange}
                        onClick={()=>this.setState({showDate:true})} type="value"
                        className="date_picker_input"/>
                    <i className="iconfont iconguanbi1" style={{fontSize:"14px",color:"#7d7d7d",lineHeight:"37px", cursor:"pointer",marginLeft:"-60px"}}
